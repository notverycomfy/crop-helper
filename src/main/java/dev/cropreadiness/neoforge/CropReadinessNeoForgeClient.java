...
public final class CropReadinessNeoForgeClient {
    ...
    @SubscribeEvent
    static void suppressVanillaOutline(ExtractBlockOutlineRenderStateEvent event) {
        Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.player != null && holdingHoe(minecraft.player)
                && classify(event.getLevel(), event.getBlockPos(), event.getBlockState()) != 0) {
