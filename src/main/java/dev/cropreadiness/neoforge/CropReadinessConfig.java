...
final class CropReadinessConfig {
    ...
    static void load() {
        ...
        if (Files.isRegularFile(FILE)) {
            try (Reader reader = Files.newBufferedReader(FILE)) {
                ...
                CropReadinessNeoForge.LOGGER.warn("Could not read Crop Helper configuration; using defaults", exception);
            }
        }
        for (int i = 0; i < KEYS.length; i++) {
            COLORS[i] = parse(properties.getProperty(KEYS[i]), DEFAULTS[i]);
        }
    }
    ...
    static void save(int[] values) throws IOException {
        ...
        Properties properties = new Properties();
        for (int i = 0; i < KEYS.length; i++) {
            properties.setProperty(KEYS[i], hex(COLORS[i]));
        }
        Files.createDirectories(FILE.getParent());
        try (Writer writer = Files.newBufferedWriter(FILE)) {
            properties.store(writer, "Crop Helper outline colors");
        }
    }

    static int color(int index) {
        return 0x88000000 | COLORS[index];
    }
